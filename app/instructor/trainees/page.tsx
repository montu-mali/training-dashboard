...
export default function TraineesPage() {
  ...
  const [selectedTrainee, setSelectedTrainee] = useState<TraineeWithProgress | null>(null)
    ...
    try {
      ...
      if (usersResponse.ok && assignmentsResponse.ok) {
        ...
        const traineesWithProgress = traineeUsers.map((trainee: any) => {
          const traineeAssignments = assignments.filter((assignment: any) => assignment.traineeId === trainee.id)
          const totalModules = traineeAssignments.length
          const completedModules = traineeAssignments.filter((assignment: any) => assignment.isCompleted).length
  ...
  const fetchModules = async () => {
    try {
      const response = await fetch(`/api/modules?instructorId=${user?.id}`)
            ...
            trainee.id === selectedTrainee.id
              ? {
                  ...
                  progressPercentage: Math.round(
                    (trainee.completedModules / (trainee.totalModules + selectedModules.length)) * 100,
                  ),
                }
              : trainee,
          ),
    ...
    try {
      ...
      if (response.ok) {
        return data.assignments.map((assignment: any) => {
          const module = availableModules.find((m) => m.id === assignment.moduleId)
          return {
            ...module,
  ...
  const getAvailableModulesForTrainee = async (traineeId: string) => {
    try {
      ...
      const data = await response.json()
      if (response.ok) {
        const assignedModuleIds = data.assignments.map((assignment: any) => assignment.moduleId)
        return availableModules.filter((module) => !assignedModuleIds.includes(module.id))
      }
    } catch (error) {
      ...
      <DashboardLayout>
        <div className="space-y-6">
          <div>
            ...
            <p className="text-muted-foreground">Manage your trainees and assign training modules</p>
          </div>
                  ...
                  <div className="flex items-start justify-between">
                    ...
                    <Dialog>
                      ...
                      <DialogContent className="sm:max-w-[525px]">
                        <DialogHeader>
                          <DialogTitle>Assign Modules to {trainee.name}</DialogTitle>
                          <DialogDescription>Select training modules to assign to this trainee.</DialogDescription>
                        </DialogHeader>
                        <div className="space-y-4">
                          ...
                          <div className="flex justify-end space-x-2">
                            ...
                            <Button
                              ...
                              disabled={selectedModules.length === 0 || isAssigning}
                            >
                              {isAssigning ? (
                                <>
          ...
          {trainees.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="mt-1 text-sm text-gray-500">No trainees have signed up yet.</p>
            </div>
          )}
        </div>
      </DashboardLayout>
